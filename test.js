...
test('MANCO', () => {
	...
	test('INICO', () => {
		...
		const chess = new Chess();
		chess.comment('starting position');
		expect(chess.comment()).toEqual('starting position');
		expect(unpack(chess.comments())).toEqual([
	...
	test('FMCOM', () => {
		...
		chess.move('e4');
		const e4 = chess.fen();
		chess.comment('good move');
		expect(chess.comment()).toEqual('good move');
		expect(unpack(chess.comments())).toEqual([
	...
	test('LMCOM', () => {
		...
		chess.move('e4');
		chess.move('e6');
		chess.comment('dubious move');
		expect(chess.comment()).toEqual('dubious move');
		expect(unpack(chess.comments())).toEqual([
	...
	test('DELCM', () => {
		// delete comment
		const chess = new Chess();
		expect(chess.comment(null)).toBe(undefined);
		expect(chess.comments(null)).toEqual([]);
		const initial = chess.fen();
		chess.comment('starting position');
		chess.move('e4');
		const e4 = chess.fen();
		...
		expect(chess.comment(null)).toEqual('dubious move');
		expect(chess.pgn()).toEqual('{starting position} 1. e4 {good move} e6');
		expect(chess.comment(null)).toBe(undefined);
		expect(unpack(chess.comments(null))).toEqual([
			{ fen: initial, comment: 'starting position' },
			{ fen: e4, comment: 'good move' },
		]);
	...
	test('PRNCM', () => {
		...
		const chess = new Chess();
		chess.move('e4');
		chess.comment('tactical');
		chess.takeback();
		chess.move('d4');
		chess.comment('positional');
		expect(unpack(chess.comments())).toEqual([
	...
	test('CLRCM', () => {
		...
		t(chess => chess.reset());
		t(chess => chess.clear());
		t(chess => chess.fen(chess.fen()));
		t(chess => chess.pgn('1. e4'));
	});
});
...
test('LOCMS', () => {
	...
	tests.forEach(t => {
		test(t.name, () => {
			const chess = new Chess();
			chess.pgn(t.input);
			expect(chess.pgn()).toEqual(t.output);
		});
	});
});
...
test('VAFEN', () => {
	...
	for (const position of positions) {
		test('`' + position.fen + '` (valid: ' + (position.error_number == 0) + ')', () => {
			const result = Chess.validate(position.fen);
			assert(result == (position.error_number == 0));
		});
	}
});
...
test('HISTR', () => {
	...
	const tests = [
		{
			...
			moves: [
				...
				'Bh4', 'b6', 'cxd5', 'Nxd5', 'Bxe7', 'Qxe7', 'Nxd5', 'exd5', 'Rc1', 'Be6', 'Qa4',
				'c5', 'Qa3', 'Rc8', 'Bb5', 'a6', 'dxc5', 'bxc5', 'O-O', 'Ra7', 'Be2', 'Nd7',
				'Nd4', 'Qf8', 'Nxe6', 'fxe6', 'e4', 'd4', 'f4', 'Qe7', 'e5', 'Rb8', 'Bc4',
				'Kh8', 'Qh3', 'Nf8', 'b3', 'a5', 'f5', 'exf5', 'Rxf5', 'Nh7', 'Rcf1', 'Qd8',
				'Qg3', 'Re7', 'h4', 'Rbb7', 'e6', 'Rbc7', 'Qe5', 'Qe8', 'a4', 'Qd8', 'R1f2',
				'Qe8', 'R2f3', 'Qd8', 'Bd3', 'Qe8', 'Qe4', 'Nf6', 'Rxf6', 'gxf6', 'Rxf6',
				'Kg8', 'Bc4', 'Kh8', 'Qf4',
			],
		},
		{
	...
	for (let i = 0; i < tests.length; i++) {
		...
		test(i, () => {
			...
			if (t.fen != chess.fen()) passed = false;
			else if (history.length != t.moves.length) passed = false;
			else {
				for (let j = 0; j < t.moves.length; j++) {
					if (!t.verbose) {
...
test('BOARD', () => {
	...
	for (const t of tests) {
		test('`' + t.fen + '`', () => {
			...
			expect(chess.board).toEqual(t.board);
		});
	}
});
...
test('REGTS', () => {
	...
	test('PMTOK', () => {
		// placing more than one king
		const chess = new Chess('N3k3/8/8/8/8/8/5b2/4K3 w - - 0 1');
		reject(chess.set({ type: 'k', color: 'w' }, 'a1'));
		chess.set({ type: 'q', color: 'w' }, 'a1');
		chess.remove('a1');
		expect(chess.moves().join(' ')).toBe('Kd2 Ke2 Kxf2 Kf1 Kd1');
	});
	test('SFALW', () => {
		// SetUp and FEN should be accepted in load_pgn white
		const chess = new Chess();
		const pgn = [
			'[SetUp "1"]',
		...
		const result = chess.pgn(pgn.join('\n'));
		assert(result);
		expect(chess.fen()).toBe('7k/5K2/7R/8/8/8/8/8 b KQkq - 1 1');
	});
	test('SFALB', () => {
		// SetUp and FEN should be accepted in load_pgn black
		const chess = new Chess();
		const pgn = [
			'[SetUp "1"]',
	...
	test('WMAFW', () => {
		// Wrong movement number after setting a position via FEN white
		const chess = new Chess();
		chess.fen('4r3/8/2p2PPk/1p6/pP2p1R1/P1B5/2P2K2/3r4 w - - 1 45');
		chess.move('f7');
		const result = chess.pgn();
		expect(result.match(/(45\. f7)$/)[0]).toBe('45. f7');
	});
	test('WMAFB', () => {
		// Wrong movement number after setting a position via FEN black
		const chess = new Chess();
		chess.fen('4r3/8/2p2PPk/1p6/pP2p1R1/P1B5/2P2K2/3r4 b - - 1 45');
		chess.move('Rf1+');
		const result = chess.pgn();
		expect(result.match(/(45\. \.\.\. Rf1\+)$/)[0]).toBe('45. ... Rf1+');
	});
	test('DCHIS', () => {
		// should not clear headers if PGN contains SetUp and FEN tags
		const pgn = [
			'[Event "Test Olympiad"]',
			'[Site "Earth"]',
			'[Date "????.??.??"]',
	...
	test('CLEAR', () => {
		// clear() should clear the chess and delete all
		// headers with the exception of SetUp and FEN
		const pgn = [
			'[Event "Test Olympiad"]',
			'[Site "Earth"]',
			'[Date "????.??.??"]',
	...
	test('SSILL', () => {
		// sloppy settings allows illegal moves
		const chess = new Chess('4k3/8/8/8/8/4p3/8/4K3 w - - 0 1');
		expect(chess.move('e1f2', { sloppy: true })).toBe(null);
	});
	test('EMPTY', () => {
		// playing a move on an empty chess throws an error
		const chess = new Chess('8/8/8/8/8/8/8/8 w KQkq - 0 1');
		expect(chess.move('e4')).toBe(null);
	});
	test('LOADD', () => {
		// load_pgn duplicate last move if it has a comment
		const history = [
			'e4', 'e5', 'Nf3', 'Nc6', 'Bb5',
			'd6', 'd4', 'Bd7', 'Nc3', 'Nf6', 'Bxc6',
		];
		// trailing comment - no end of game marker
		const chess = new Chess();
		let result = chess.pgn(
			'1. e4 e5 2. Nf3 Nc6 3. Bb5 d6 ' +
			'4. d4 Bd7 5. Nc3 Nf6 6. Bxc6 {comment}',
		);
		assert(result);
		expect(chess.history()).toEqual(history);
		expect(chess.header()['Result']).toBe(undefined);
	...
	test('PGNSM', () => {
		// pgn should not generate sloppy moves
		const chess = new Chess();
		chess.pgn('1. e4 d5 2. Nf3 Nd7 3. Bb5 Nf6 4. O-O');
		expect(chess.pgn()).toBe('1. e4 d5 2. Nf3 Nd7 3. Bb5 Nf6 4. O-O');
	});
	test('STRCT', () => {
		// strict parser should always run before sloppy
		const chess = new Chess();
		// these test examples are lifted from the github issue
		chess.fen(
			'r4rk1/4nqpp/1p1p4/2pPpp2/bPP1P3/R1B1NQ2/P4PPP/1R4K1 w - - 0 28',
		);
		expect(chess.move('bxc5')).toBeNot(null);
		chess.fen(
			'r4rk1/4nqpp/1p1p4/2pPpp2/bPP1P3/R1B1NQ2/P4PPP/1R4K1 w - - 0 28',
		);
		expect(chess.move('bxc5', { sloppy: true })).toBeNot(null);
		// over-disambiguation without sloppy should fail
		chess.fen(
			'rnbqk2r/p1pp1ppp/1p2pn2/8/1bPP4/2N1P3/PP3PPP/R1BQKBNR w KQkq - 0 5',
		);
		expect(chess.move('Nge2')).toBe(null);
		// over-disambiguation with sloppy should pass
		chess.fen(
			'rnbqk2r/p1pp1ppp/1p2pn2/8/1bPP4/2N1P3/PP3PPP/R1BQKBNR w KQkq - 0 5',
		);
		expect(chess.move('Nge2', { sloppy: true })).toBeNot(null);
	});
	test('IWAHT', () => {
		// ignore whitespace after header tag (load_pgn)
		const chess = new Chess();
		const pgn = `
		[white "player a"]
			[black "player b"]
	...
	test('IWLAH', () => {
		// ignore whitespace in line after header (load_pgn)
		const chess = new Chess();
		const pgn = `
		[white "player a"]
			[black "player b"]
...
test('ASCII', () => {
	const chess = new Chess('k7/p2PR1p1/7p/2p1N3/1n1r3P/1P2K3/P7/8 b - - 2 44');
	test('WHITE', () => {
		expect(chess.ascii(WHITE)).toBe([
			'   ╔════════════════════════╗',
			' 8 ║ ♚  ·  ·  ·  ·  ·  ·  · ║',
			' 7 ║ ♟  ·  ·  ♙  ♖  ·  ♟  · ║',
			' 6 ║ ·  ·  ·  ·  ·  ·  ·  ♟ ║',
